...
| D6 | `crm` `people`/`companies`/`notes` entities | Flows use `clients`; people/companies UI minimal | Routes still mounted; module-crm still renders them (not fully traced) | Needs verification | Check UI usage + collection counts (read-only count query in a follow-up with owner consent) |
| D7 | `dashboardstats` schema + dummy metrics | Docs describe dummy data | Controller returns stats; frontend module-dashboard 1.5k lines (has real UI?) — not fully traced | Needs verification | Read module-dashboard data sources |
...
| D9 | `apps/module-projects` `components/ui` vs `packages/ui` overlap | Two UI component sets | Not diffed in depth | Needs verification | Diff component names/usages |
