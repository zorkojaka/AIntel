...
| D4 | `apps/module-projects` legacy embedded-offer UI paths | Legacy embedded flow superseded by OfferVersion endpoints | AIN-P2-01 grep: module-projects uses OfferVersion `/offers` endpoints; legacy embedded offer controller actions are unmounted and guarded with 410 if re-mounted | Confirmed frozen | Observe `legacy.project_embedded_write` logs; archive guarded code after owner OK |
| D5 | `project.controller.ts` legacy offer confirm/select + `createPurchaseOrders` + embedded deliveryNotes flow (lines ~1042–1226) | Superseded by `logistics.confirmOffer` + MaterialOrder | AIN-P2-01: still-mounted `receiveDelivery` route and unmounted legacy offer actions return 410 before DB writes and log `legacy.project_embedded_write` | Confirmed frozen | Archive guarded code after owner OK |
