...
- Projects modul (`/projects`) hrani projekte v spominu in podpira tok ponudb: `GET /projects` vrne povzetke, `GET /projects/:id` vrne postavke, ponudbe, naročilnice, dobavnice, timeline in predloge. `POST /projects` ustvari nov projekt, `POST /projects/:id/offers` doda verzijo, `POST /projects/:id/offers/:offerId/send|confirm|cancel|select` vodijo status ponudbe, `POST /projects/:id/deliveries/:deliveryId/receive` potrdi dobavo, `POST /projects/:id/status` in `POST /projects/:id/signature` posodobita status. Frontend gumbi v `apps/module-projects` kličejo te API-je in osvežijo `TimelineFeed`.
