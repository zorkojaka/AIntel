    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <Badge className={
                    template.category === "offer" ? "bg-blue-100 text-blue-700" :
                    template.category === "invoice" ? "bg-green-100 text-green-700" :
                    "bg-purple-100 text-purple-700"
                  }>
                    {template.category === "offer" ? "Ponudba" :
                     template.category === "invoice" ? "Račun" :
                     "Delovni nalog"}
                  </Badge>
                  {template.isDefault && (
        ...
        {templates.length === 0 && (
          <Card className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">Še ni ustvarjenih predlog</p>
            <Button onClick={handleCreate}>
              <Plus className="w-4 h-4 mr-2" />
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingTemplate ? "Uredi predlogo" : "Nova predloga"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              <div className="border rounded-[var(--radius-card)] p-4 bg-muted/30 max-h-60 overflow-y-auto">
                <div className="grid grid-cols-2 gap-2">
                  {PLACEHOLDER_INFO.map((ph) => (
                    <button
                      key={ph.key}
                      onClick={() => handleInsertPlaceholder(ph.key)}
                      className="text-left p-2 rounded hover:bg-background transition-colors group"
                    >
                      <code className="text-xs text-primary group-hover:underline">{ph.key}</code>
            ...
            <div className="flex gap-2 justify-end pt-4">
              ...
              <Button onClick={handleSave}>
                {editingTemplate ? "Posodobi" : "Ustvari"}
              </Button>
            </div>
          </div>
        </DialogContent>
