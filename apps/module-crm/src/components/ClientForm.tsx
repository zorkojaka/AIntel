...
const CITY_TO_CODE = Object.entries(POSTAL_CODE_LOOKUP).reduce<Record<string, string>>((acc, [code, city]) => {
...
export function ClientForm({ open, client, mode = 'create', onClose, onSubmit, onSuccess }: ClientFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const postalLine = parsed.postalCode
        ? `${parsed.postalCode} ${parsed.postalCity ?? ''}`.trim()
        : parsed.postalCity;
  ...
  return (
    <div className="client-modal__overlay" role="dialog" aria-modal="true">
      <div className="client-modal__dialog">
        <header className="client-modal__header">
          <h2>{title}</h2>
          <button type="button" className="client-modal__close" aria-label="Zapri" onClick={onClose}>
            ×
          </button>
        </header>
