...
export function CategorySettingsPanel() {
  ...
  const activeProductCount = settings
    .filter((setting) => setting.level === 2 && setting.isActive)
    ...
    setSettings((prev) =>
      prev.map((setting) => {
        ...
        if (setting.path === 'Videonadzorni sistemi') {
          isActive = true;
          priority = 1;
        } else if (recommendedVideoSubCategories.has(setting.path)) {
          isActive = true;
        } else if (setting.path === 'Protivlomni sistemi') {
          isActive = true;
          priority = 1;
        } else if (setting.topLevel === 'Protivlomni sistemi' && setting.level === 2 && !excludedAlarmSubCategories.has(setting.path)) {
          isActive = true;
