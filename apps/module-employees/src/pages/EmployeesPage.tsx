...
export function EmployeesPage() {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    const filtered = term
      ? visible.filter((employee) => {
          const haystack = `${employee.name} ${employee.company ?? ''}`.toLowerCase();
  ...
  const handleDelete = async (employee: Employee) => {
    ...
    const confirmed = window.confirm(`Res želite izbrisati zaposlenega "${employee.name}"?`);
    if (!confirmed) return;
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            ...
            <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm">
              ...
              <input
                type="search"
                placeholder="Išči po imenu ali podjetju"
